...
const SEASONS = [
  ['Full Season',    '32 weeks', 'border-brand-500'],
  ['Half Season',    '16 weeks', 'border-brand-500/60'],
  ['Quarter Season', '8 weeks',  'border-brand-300/40'],
];
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="space-y-4 mb-12">
        {STEPS.map(s => (
          <div key={s.n} className="bg-white border border-gray-200 rounded-xl p-5 flex gap-4">
            <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center font-black text-white text-base flex-shrink-0">{s.n}</div>
            <div>
              ...
              <ul className="space-y-1">
                {s.bullets.map((b, i) => (
                  <li key={i} className="text-slate-500 text-xs flex gap-1.5 items-start">
                    <ChevronRight className="w-3 h-3 text-brand-700 flex-shrink-0 mt-0.5" />{b}
                  </li>
                ))}
              </ul>
            </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          <h3 className="text-lg font-bold mb-4 text-brand-600">Betting Rules</h3>
          <ul className="space-y-3 text-sm text-slate-700">
            {BETTING_RULES.map(([b, r], i) => (
              <li key={i} className="flex gap-2 items-start">
                <ChevronRight className="w-3.5 h-3.5 text-brand-700 flex-shrink-0 mt-0.5" />
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          <h3 className="text-lg font-bold mb-4 text-brand-600">Season Lengths</h3>
      ...
      <div className="bg-white border border-gray-300 rounded-xl p-6 mb-8">
        <h3 className="text-lg font-bold mb-3 text-brand-600">The Punting Week</h3>
      ...
      <div className="bg-white border border-gray-300 rounded-xl p-6 text-center">
        ...
        <button
          ...
          className="bg-gold-500 hover:bg-gold-400 text-brand-950 px-8 py-3 rounded-xl font-bold transition-all hover:scale-105"
        >
          Create Team Now
        </button>
      </div>
