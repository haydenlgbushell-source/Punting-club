...
async function callClaudeWithSearch(prompt) {
  ...
  for (let turn = 0; turn < 8; turn++) {
    ...
    if (data.stop_reason === 'tool_use') {
      ...
      // The client must continue with empty content so Anthropic injects the real results.
      // Passing a non-empty string (e.g. 'Search executed.') blocks the actual search data.
